...
const TAG_LENGTH = 16; // 128 bits for AES-GCM
// The header for each encrypted block contains IV (12 bytes) plus space for the auth tag (16 bytes)
const ENCRYPTION_HEADER_SIZE = IV_LENGTH + TAG_LENGTH;
...
export class OPFSEncryptedVFS extends FacadeVFS {
  ...
  static async create(name, module, options) {
    ...
    const vfs = new OPFSEncryptedVFS(name, module);
    ...
    vfs.encryptionKey = await crypto.subtle.deriveKey(
      {
        name: "PBKDF2",
      ...
      const encryptedBuffer = await crypto.subtle.encrypt(
        {
          ...
          iv,
          tagLength: TAG_LENGTH * 8 // Tag length in bits
        },
        this.encryptionKey,
        data
      );
      
      // Create a result buffer that includes [IV][Encrypted Data + Auth Tag]
      const resultBuffer = new Uint8Array(IV_LENGTH + encryptedBuffer.byteLength);
      
      // Copy IV and encrypted data into result buffer
      resultBuffer.set(iv, 0);
      resultBuffer.set(new Uint8Array(encryptedBuffer), IV_LENGTH);
  ...
  async #decryptData(encryptedData) {
    try {
      ...
      // Extract the encrypted data (including the auth tag)
  ...
  jRead(fileId, pData, iOffset) {
    try {
      ...
      // Calculate the offset with encryption header
      // Each block of data has its own IV and tag
      // For simplicity, we're reading the entire block and then extracting what we need
      const adjustedOffset = iOffset + Math.floor(iOffset / pData.byteLength) * ENCRYPTION_HEADER_SIZE;
      
      // Create a buffer to hold the encrypted data (original data + encryption header)
      const encryptedBuffer = new Uint8Array(pData.byteLength + ENCRYPTION_HEADER_SIZE);
      const bytesRead = accessHandle.read(encryptedBuffer, { at: adjustedOffset });
      ...
      if (bytesRead > 0) {
        ...
        // Create a shared array buffer to signal completion
        const sharedBuffer = new SharedArrayBuffer(4);
        const syncSignal = new Int32Array(sharedBuffer);
        
        decryptionPromise.then(result => {
          decryptedData = result;
          Atomics.store(syncSignal, 0, 1);
          Atomics.notify(syncSignal, 0);
        }).catch(e => {
          console.error("Decryption failed:", e);
          Atomics.store(syncSignal, 0, 2);
          Atomics.notify(syncSignal, 0);
        });
        
        // Wait for decryption to complete
        Atomics.wait(syncSignal, 0, 0);
        
        const status = Atomics.load(syncSignal, 0);
        if (status === 1 && decryptedData) {
          // Copy the decrypted data to the output buffer
          pData.set(decryptedData.subarray(0, Math.min(pData.byteLength, decryptedData.byteLength)));
          
          // If we read less than requested, fill the rest with zeros
          if (decryptedData.byteLength < pData.byteLength) {
            pData.fill(0, decryptedData.byteLength);
  ...
  jWrite(fileId, pData, iOffset) {
    try {
      ...
      const status = Atomics.load(syncSignal, 0);
      if (status === 1 && encryptedData) {
        // Calculate the offset with encryption header
        const adjustedOffset = iOffset + Math.floor(iOffset / pData.byteLength) * ENCRYPTION_HEADER_SIZE;
        
        // Write the encrypted data
        const nBytes = accessHandle.write(encryptedData, { at: adjustedOffset });
        if (nBytes !== encryptedData.byteLength) throw new Error('short write');
      } else {
  ...
  jTruncate(fileId, iSize) {
    try {
      ...
      // Calculate the adjusted size with encryption headers
      // Each block has its own encryption header
      const blockSize = 4096; // Assume a default block size
      const blocksInFile = Math.ceil(iSize / blockSize);
      const adjustedSize = iSize + (blocksInFile * ENCRYPTION_HEADER_SIZE);
      
      accessHandle.truncate(adjustedSize);
      return VFS.SQLITE_OK;
    } catch (e) {
      this.lastError = e;
  ...
  jFileSize(fileId, pSize64) {
    try {
      const file = this.mapIdToFile.get(fileId);
      const accessHandle = file.accessHandle || file.persistentFile.accessHandle;
      const encryptedSize = accessHandle.getSize();
      
      // Calculate the actual data size by removing encryption headers
      // Each block has its own encryption header
      const blockSize = 4096; // Assume a default block size
      const blocksInFile = Math.ceil(encryptedSize / (blockSize + ENCRYPTION_HEADER_SIZE));
      const actualSize = Math.max(0, encryptedSize - (blocksInFile * ENCRYPTION_HEADER_SIZE));
      
      pSize64.setBigInt64(0, BigInt(actualSize), true);
      return VFS.SQLITE_OK;
    } catch (e) {
      this.lastError = e;
